            ...
            return (
              <div key={msg.id} className={`${styles.msgRow} ${mine ? styles.msgRowMine : ''}`}>
                ...
                <div className={`${styles.msgBubble} ${mine ? styles.msgBubbleMine : ''}`}>
                  {!mine ? <Link to={`/profile/${msg.senderUsername || msg.senderId}`}><b>{msg.senderName}</b>{msg.senderUsername ? <span style={{ fontSize: '0.7rem', color: '#88949e', marginLeft: '0.2rem' }}>@{msg.senderUsername}</span> : null}</Link> : null}
          ...
          <div className={styles.memberList}>
            {selectedGroup?.members.slice(0, 8).map((member) => (
              <div key={member.userId} className={styles.memberItem}>
                ...
                <div>
                  <Link to={`/profile/${member.username || member.userId}`}><b>{member.fullName}</b></Link>
                  {member.username ? <small style={{ fontSize: '0.65rem', color: '#88949e', display: 'block' }}>@{member.username}</small> : null}
