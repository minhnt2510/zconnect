...
export class FriendshipController {
  ...
  @UseGuards(JwtAuthGuard)
  getUserByUsername(@Param('username') username: string, @Req() req: any) {
    return this.friendshipService.getUserProfileByUsername(
      req.user.sub,
  ...
  @Get('users/:userId')
  @UseGuards(JwtAuthGuard)
  getUserById(@Param('userId') userId: string, @Req() req: any) {
  ...
  @UseGuards(JwtAuthGuard)
  getBlockStatus(@Param('userId') userId: string, @Req() req: any) {
    return this.friendshipService.isUserBlocked(
      req.user.sub,
      parseInt(userId, 10),
    );
  }
  ...
  @Get('calls')
  @UseGuards(JwtAuthGuard)
  getCallHistory(@Query('limit') limit?: string) {
    return { calls: [] };
  }
