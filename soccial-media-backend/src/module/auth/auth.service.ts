...
export class AuthService {
  ...
  async checkUsername(username: string) {
    ...
    const exists = await this.userService.findOneByUsername(normalized);
  ...
  async updateMe(
    ...
    if (data.username !== undefined) {
      ...
      if (username.length < 3 || username.length > 30) {
        throw new BadRequestException('Username phải từ 3 đến 30 ký tự');
      }
      if (!/^[a-zA-Z0-9._]+$/.test(username)) {
        throw new BadRequestException('Username không hợp lệ');
      }
      const existing = await this.userService.findOneByUsername(username);
  ...
  async updateCoverUrl(userId: number, coverUrl: string) {
    await this.userService.update(userId, { coverUrl });
    const user = await this.userService.findOne(userId);
    if (!user) {
      throw new UnauthorizedException('Người dùng không tồn tại');
    }
  ...
  async handleGoogleLogin(googleUser: {
    ...
    if (!googleUser.email) {
      throw new BadRequestException('Google email khong hop le');
    }
